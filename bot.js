...
const {Message, StringSession, Image, Video} = require('./cocosir/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('cocosir WORKING ' + config.WORKTYPE + ' 𝚗𝚘𝚠 👻'));
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '__-COCO-007_☠☠_[error] ' +
                                    '\n\n*👻 ' + error + '*\n'
                                    , MessageType.text);
                            }
                        }
